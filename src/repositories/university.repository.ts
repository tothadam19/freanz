...
> {
  ...
  async searchUnis(
    ...
    for (const uni of query) {
      for (const faculty of uni.faculty) {
        if (faculty.major) {
          for (const major of faculty.major) {
            if (major.majorName.toLowerCase().includes(regexp.toLowerCase())) responseArray.push({
        ...
        if (faculty.facultyName.toLowerCase().includes(regexp.toLowerCase()) && !faculty.facultyName.toLowerCase().includes('nem rendelkezik')) responseArray.push({
          facultyID: faculty.facultyID,
          facultyName: faculty.facultyName,
          universityID: uni.universityID,
          universityName: uni.universityName,
      ...
      if (uni.universityName.toLowerCase().includes(regexp.toLocaleLowerCase())) responseArray.push({
        universityID: uni.universityID,
        universityName: uni.universityName,
      })
    }
